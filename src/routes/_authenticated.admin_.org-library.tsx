...
function UploadDialog({ viewerEmail, onClose }: { viewerEmail: string; onClose: () => void }) {
  ...
  const [docType, setDocType] = useState<DocType>("source_doc");
  ...
  function submit(e: React.FormEvent) {
    ...
    if (!rights) { toast.error("Rights attestation required"); return; }
    if (isLinkKind && !externalUrl.trim()) { toast.error("Link URL required"); return; }
    if (!isLinkKind && !fileName.trim()) { toast.error("File name required"); return; }
    if (makeVisible && !phi) { toast.error("PHI attestation required to make ATE-visible at upload."); return; }
    ...
    addAsset({
      ...
      asset_kind: kind,
      file_name: isLinkKind ? null : fileName.trim(),
      ...
      related_ask_ids: [],
      visibility: makeVisible && phi ? "org_ate_visible" : "admin_only_source",
    });
  ...
  return (
    <div className="fixed inset-0 z-40 bg-foreground/40 backdrop-blur-sm flex items-end md:items-center justify-center p-4" onClick={onClose}>
      <form
        onSubmit={submit}
        className="bg-card rounded-3xl border border-border shadow-elevated w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto space-y-3"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="font-display font-semibold flex items-center gap-2"><Upload className="size-4 text-primary" /> Add organization asset</div>

        <Field label="Title"><input value={title} onChange={(e) => setTitle(e.target.value)} className={inputCls} required /></Field>
        <Field label="Summary"><textarea value={summary} onChange={(e) => setSummary(e.target.value)} className={`${inputCls} h-16 py-2`} /></Field>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <Field label="Workflow area"><input value={workflow} onChange={(e) => setWorkflow(e.target.value)} className={inputCls} /></Field>
          <Field label="System / vendor label"><input value={systemLabel} onChange={(e) => setSystemLabel(e.target.value)} className={inputCls} /></Field>
        ...
        {isLinkKind ? (
          <Field label="External URL"><input value={externalUrl} onChange={(e) => setExternalUrl(e.target.value)} className={inputCls} placeholder="https://…" /></Field>
        ) : (
          <Field label="File name"><input value={fileName} onChange={(e) => setFileName(e.target.value)} className={inputCls} placeholder="my-doc.pdf" /></Field>
        )}
        ...
        <label className="flex items-start gap-2 rounded-xl border border-border bg-secondary/40 p-3 cursor-pointer">
          ...
          <div className="text-xs">
            ...
            <div className="text-muted-foreground">The organization confirms it has permission to use this content for internal training / support.</div>
          </div>
        </label>
        <label className="flex items-start gap-2 rounded-xl border border-border bg-secondary/40 p-3 cursor-pointer">
          ...
          <div className="text-xs">
            ...
            <div className="text-muted-foreground">This content does not contain PHI or has been approved for internal use.</div>
          </div>
        </label>
        <label className="flex items-start gap-2 rounded-xl border border-border bg-secondary/40 p-3 cursor-pointer">
          <input type="checkbox" checked={makeVisible} onChange={(e) => setMakeVisible(e.target.checked)} className="mt-0.5 size-4 accent-primary" />
          <div className="text-xs">
            <div className="font-medium">Make ATE-visible on upload</div>
            <div className="text-muted-foreground">Otherwise saves as admin-only source until you approve it.</div>
          </div>
        </label>

        <div className="flex justify-end gap-2 pt-2">
          <button type="button" onClick={onClose} className="h-10 px-4 rounded-xl border border-border text-sm">Cancel</button>
          <button type="submit" className="h-10 px-4 rounded-xl bg-primary text-primary-foreground text-sm font-medium">Save</button>
        </div>
      </form>
    </div>
  );
