...
export const LAUNCH_LIBRARY: LaunchEntry[] = [
  ...
  },
  {
    ...
    related_ids: ["vc_orders_full_workflow", "p2", "s1"],
    sanitized_approved: true, status: "published",
  },
  {
    id: "ll_scope_orders",
    ...
    domains: k("orders", "order entry", "scope"),
    phases: k("cutover day 0", "stabilization week 1", "optimization"),
    urgency: 2,
    escalation: 1,
    action: "place",
    is_deep_flow: false,
    nav_trail: "Confirm patient/encounter -> Open Orders / order entry -> Pick order type -> Review details -> Sign or route",
    visual_url: "/visual-guides/orders-scope-overview.svg",
    visual_callouts: [
      "1 - Confirm patient and encounter.",
      "2 - Open Orders / order entry activity.",
      "3 - Search or pick the order type (med, lab, imaging, procedure, set).",
      "4 - Review order details (dose, priority, routing, diagnosis, encounter).",
      "5 - Sign or route to the right owner.",
    ],
    first90: [
      "Confirm the patient and the correct encounter.",
      "Open the order-entry area (Orders activity).",
      "Pick the exact order type before troubleshooting.",
    ],
    whatToSay: [
      "'Let's first find out what kind of order we're dealing with, then we'll take the right path.'",
    ],
    whatToCheck: [
      "Common order paths: Can't find Orders | Place a new order | Order won't sign | Order is pending | Order is missing | Wrong department/context | Order set / SmartSet issue.",
      "By type: Lab | Imaging | Medication | Procedure | Inpatient | Ambulatory.",
      "Patient context, encounter, location, role, and signing authority.",
    ],
    whenToEscalate: "If you have confirmed patient, encounter, context, and order type and the order still cannot be placed, signed, or routed, escalate with patient context, encounter, order type, exact step that fails, error text, and callback.",
    walkthrough: [
      "Confirm patient and encounter.",
      "Open the Orders activity from the chart.",
      "Pick the exact order type or path below, then drill into the specific workflow.",
      "Path: 'I can't find Orders' -> open the Orders activity, check role/context.",
      "Path: 'I need to place an order' -> order entry workflow.",
      "Path: 'I can't sign an order' -> signing failure workflow.",
      "Path: 'Order is missing' -> encounter/context workflow.",
      "Path: 'Order is pending' -> unsigned/initiated workflow.",
      "Path: 'Order went to the wrong place' -> routing/department workflow.",
      "Path: 'Order set / SmartSet issue' -> SmartSet workflow.",
      "Path: 'Lab / Imaging / Medication / Procedure' -> specific order-type workflow.",
    ],
    ifThatFails: [
      "If you cannot identify the order type, stay on order entry and capture exact wording the user expects.",
      "If the user is in the wrong department/context, switch to the context/profile workflow first.",
      "If the order is built but cannot be placed by the user's role, escalate to build/personalization owner.",
    ],
    keywords: k(
    ...
    related_ids: [
      "vc_orders_full_workflow",
      "ll_order_entry",
      "ll_signature_failure",
      "ll_order_missing_due_to_encounter_context",
      "ll_order_status_unsigned_initiated",
      "ll_smartset_order_set_missing",
      "ll_smartset_section_or_order_not_selected",
      "ll_radiant_imaging_order_status",
      "ll_diagnosis_needed_for_order",
      "ll_order_favorites_or_personal_list_missing",
      "ll_discontinue_cancel_order_safely",
      "ll_order_locked_after_sign",
      "ll_p12r2_wrong_department_context",
      "p2",
      "s1",
    ],
    sanitized_approved: true,
  ...
  },
  {
    ...
    keywords: k("schedule", "scheduling", "appointment", "columns", "column", "change columns", "schedule line", "line", "grid", "view", "personalize", "add column", "remove column", "show column", "column settings", "schedule personalization", "schedule columns personalization"),
  ...
  },
  {
    ...
    related_ids: ["vc_smartlink_blank_or_wrong", "p24", "c18", "v20"],
    sanitized_approved: true,
    status: "published",
  },
  {
  ...
  },
  {
    ...
    keywords: k("print prescription", "print prescriptions", "prescription printer", "rx printer", "wrong prescription printer", "troy printer", "t printer", "how to print prescriptions", "printer for prescription", "prescription not printing", "rx not printing", "prescription printed wrong printer", "print rx", "paper prescription printer", "secure prescription printer", "secure rx printer"),
  ...
  },
  {
    ...
    related_ids: ["vc_notes_write_and_sign", "p30", "c24", "v26"],
    sanitized_approved: true,
    status: "published",
  },
  {
  ...
  {
    id: "ll_patient_chart_not_loading_after_admission",
    title: "Patient chart does not load after admission",
    type: "playbook",
    summary: "User can find the patient but the chart won't open after admission, transfer, registration, or context change. Confirm chart, then sort by context, status, or access before escalating.",
    roles: k("inpatient nurse", "inpatient provider", "registration", "clinical support", "all roles"),
    domains: k("patient lists", "chart access", "admission"),
    phases: k("cutover day 0", "stabilization week 1"),
    urgency: 3,
    escalation: 2,
    action: "review",
    ...
    visual_callouts: [
      "1 - Search and confirm patient identity and encounter.",
      "2 - Check admission/registration status banner.",
      "3 - Observe chart loading or blank state and capture exact error.",
      "4 - Confirm location/unit context and user role/access.",
      "5 - Package escalation: patient context, encounter, status, location, error, callback.",
    ],
    first90: [
      "Confirm the patient and encounter.",
      "Check admission/status and location context.",
      "Capture the exact error or blank state.",
    ],
    whatToSay: [
      "'Let's confirm the chart exists, then figure out whether this is context, status, or access.'",
    ],
    whatToCheck: [
      "Patient identity and the correct encounter.",
      "Admission/registration status.",
      "Location/unit context for the workstation.",
      "User role and access for that unit/encounter.",
      "Error banner text or blank-screen behavior.",
      "Whether one user or multiple users are affected.",
      "Whether the patient appears in search but not in the list/chart.",
    ],
    whenToEscalate: "If admission/registration status is wrong or missing, escalate to the registration/admission owner. If status is correct but the chart still won't load, escalate to clinical support with patient context, encounter/status, location, exact error text, and callback. Do not guess or tell the user to document elsewhere unless local downtime policy says so.",
    walkthrough: [
      "Confirm patient and correct encounter from search.",
      "Read the admission/registration status banner.",
      "Reopen chart from the correct encounter.",
      "Capture exact error text or blank state.",
      "Confirm workstation context and user access.",
    ],
    ifThatFails: [
      "Refresh the patient list/search, reopen the correct encounter, and close the loop with the user.",
      "If status or context is wrong: escalate to registration/admission owner.",
      "If status and context are correct: escalate to clinical support with full packet.",
    ],
    keywords: k(
      "patient chart not loading after admission",
      "chart not loading after admission",
      "patient chart will not open",
      "patient chart won't open",
      "chart fails to load",
      "chart spinning",
      "cannot open patient chart",
      "patient admitted but chart not opening",
      "admission chart issue",
      "chart not available after registration",
      "patient chart blank",
      "chart opens blank",
      "chart loading forever",
      "chart won't load",
      "chart not opening after admit",
      "chart not opening after transfer",
      "chart not opening after registration",
    ),
    related_ids: ["ll_patient_list", "ll_wrong_location", "ll_p13_carecompass_patient_missing"],
    sanitized_approved: true,
    status: "published",
  },
  {
  ...
  },
  {
    ...
    visual_url: "/visual-guides/medication-scan-wrong-patient-warning.svg",
    visual_callouts: [
      "1 - Scan wristband and verify patient identity.",
      "2 - Wrong-patient warning — wristband does not match chart.",
      "3 - Wrong-medication warning — package does not match the active order.",
      "4 - Route the correct owner: identity, pharmacy, or device.",
      "5 - Hold the dose — never override a safety warning.",
    ],
    first90: [
  ...
  },
  {
    ...
    vendor_family: "epic",
    action: "review",
    nav_trail: "Activity tab strip -> More / wrench / customize view -> Show hidden activity -> Save view -> Compare with peer in same role",
    first90: [
      "Look at the activity tab strip and open the More / wrench / customize view menu.",
      "Find the missing activity, tab, or report in the hidden / available list and add it back to the view.",
      "Save the view, then confirm a peer in the same role and department sees the same activity.",
    ],
    whatToSay: [
      "'Most of the time the activity is hidden in your personal view, not blocked by access — let's add it back first.'",
      "'If it's not in the hidden list and a peer in your role has it, that's a role or access issue and I'll route it.'",
    ],
    whatToCheck: [
      "Whether the activity appears in the hidden / available list on the activity tab strip.",
      "Whether a comparison user in the same role and department sees the activity.",
      "Whether the user is logged into the correct department context for that activity.",
      "Whether the user's role / template grants the activity at all.",
    ],
    whenToEscalate: "If the activity isn't in the hidden list and a peer in the same role and department does have it, send it to the access and security team with the user, role, department, and the missing activity name.",
    walkthrough: [
  ...
  },
  {
    ...
    vendor_family: "epic",
    action: "review",
    nav_trail: "Top context bar -> Department / location / login context -> Switch context -> Re-open the workflow from scratch",
    first90: [
      "Look at the top context bar and read the current department, location, and login context.",
      "Use the switch / change context option to move to the correct department or location for the work being done.",
      "Close and re-open the workflow from scratch so it loads under the new context.",
    ],
    whatToSay: [
      "'Your login department drives what you see — let's switch context to the right department before we touch anything else.'",
      "'If the right department isn't in your switch list, that's a role or location mismatch and I'll route it.'",
    ],
    whatToCheck: [
      "Department, location, and login context shown in the top context bar.",
      "Whether the user picked the right department at login (and whether they have access to switch).",
      "Whether the user's default department / location in their profile is set correctly.",
      "Whether the user's role is mapped to the location they're trying to work in.",
    ],
    whenToEscalate: "If the user can't see or pick the correct department in the switch list, send it to the access and security team with the user, role, and the missing department or location.",
    walkthrough: [
    ...
    keywords: k("wrong department", "wrong department context", "wrong login department", "logged in wrong department", "wrong context", "wrong clinic", "login context", "change context", "department switch", "logged in wrong place", "wrong location context", "session information", "profile context", "session profile", "wrong session context", "profile context troubleshooting"),
    related_ids: ["p1", "c1", "v1"],
    sanitized_approved: true,
    status: "published",
  },
  {
    ...
    related_ids: ["vc_smartlink_blank_or_wrong", "p1", "c1", "v1"],
    sanitized_approved: true,
    status: "published",
  },
  {
  ...
  },
  {
    ...
    first90: [
      ...
      "Send (or resend) the proxy activation invite and confirm the proxy's portal access status flips to active.",
    ],
    whatToSay: [
      "'I can set up proxy access once we have the right consent for this caregiver relationship on file.'",
      "'They'll get an activation invite — they need to use that link, not start a new patient account, or the access won't link to the patient.'",
    ],
    whatToCheck: [
      "The caregiver relationship type and the matching consent on file.",
      "Whether the proxy already has their own portal account (linking vs new account).",
      "The patient's account / portal status (active, deactivated, minor with age-based rules).",
      "Which proxy features are allowed for this relationship (messages, results, scheduling, billing) and whether activation has completed.",
    ],
    whenToEscalate: "If consent rules are unclear, the relationship doesn't fit a standard type, age-based access is in question, or activation never completes, send it to the patient portal support team with the relationship, the consent on file, and the proxy's access status.",
    walkthrough: [
  ...
  },
  {
    ...
    vendor_family: "epic",
    action: "review",
    nav_trail: "Patient chart -> Portal scheduling setup -> Visit type availability -> Provider / resource eligibility -> Department / location -> Scheduling rule -> Patient eligibility",
    first90: [
      "Confirm the visit type the patient is trying to book is turned on for portal self-scheduling.",
      "Check that the provider or resource is eligible for portal booking in the patient's department or location.",
      "Check the scheduling rule and patient eligibility (forms due, balance, new vs established) — if blocked, book on the patient's behalf and explain what the portal blocked.",
    ],
    whatToSay: [
      "'Not every visit type is open to portal booking — let me get this on the schedule for you now and explain what the portal blocked.'",
    ],
    whatToCheck: [
      "Whether the visit type is enabled for portal self-scheduling.",
      "Whether the provider / resource is enabled for portal booking and has open slots.",
      "Whether the department or location offers portal scheduling for that visit type.",
      "Which scheduling rule fires (decision tree, ticket, referral required).",
      "Patient eligibility flags (overdue forms, balance, new patient, age) that block portal booking.",
    ],
    whenToEscalate: "If many patients hit the same block, or the visit type / provider / location should be open and isn't, send it to the scheduling build team with the visit type, provider, location, and a few patient examples (no other PHI).",
    walkthrough: [
  ...
  },
  {
    ...
    vendor_family: "epic",
    action: "review",
    nav_trail: "Eye-care visit -> Selected exam form -> Sections / customize menu -> Add missing section -> Save personal layout -> Compare to peer in same specialty role",
    first90: [
      "Open the exam form the user has selected and look at the eye-care exam layout for that form.",
      "Open the sections / customize menu and add the missing section (slit lamp, refraction, IOP, etc.) back into the personal layout, then save.",
      "Compare with a peer in the same specialty role to confirm whether this is a personalization gap or a missing build.",
    ],
    whatToSay: [
      "'Most of the time the section is hidden in your personal eye-care layout — let's add it back before we call it a build issue.'",
      "'If it's not in the sections menu and a peer in your specialty also can't see it, that's a build escalation and I'll route it.'",
    ],
    whatToCheck: [
      "Which exam form is selected and which eye-care layout it loads.",
      "Whether the missing section appears in the sections / available list to add.",
      "Whether the user's role and specialty are mapped to the correct exam layout.",
      "Whether a peer in the same specialty role sees the section on the same form.",
    ],
    whenToEscalate: "If the section isn't available to add and peers in the same specialty also can't see it, send it to the eye-care personalization / build team with the visit type, the selected exam form, the role, and the missing section.",
    walkthrough: [
  ...
  },
  {
    ...
    action: "review",
    is_deep_flow: true,
    nav_trail: "Eye-care chart -> Imaging / media tab -> Clear filters -> Confirm study type and date range -> Check linked order / procedure -> Open in expected viewer / home view",
    first90: [
      "On the eye-care chart, open the imaging / media tab and clear any active filters.",
      "Confirm the study type, laterality, and date range cover today's image, and check that the image is linked to the right order or procedure.",
      "Open the image in the expected eye-care viewer or home view; if it's not there, check the all-media view to see if it landed under a different category.",
    ],
    whatToSay: [
      "'The image is usually there — it's typically a filter, the wrong study type, or it's linked to a different order, not a lost image.'",
    ],
    whatToCheck: [
      "Active filters on the imaging / media tab (date range, study type, laterality).",
      "Whether the image is linked to the expected order or procedure for this visit.",
      "Whether the image came in under a different study type or category than expected.",
      "Whether the expected eye-care viewer or home view is the one the user is opening, and whether their personal view hides certain study types.",
    ],
    whenToEscalate: "If the image is on the chart and linked to an order but still won't display in the expected eye-care viewer or home view, send it to the eye-care imaging team with the patient timing, the device, the study type, and the linked order.",
    walkthrough: [
  ...
  // --- Pack 13: high-value workflow upgrades (additive) ---
  {
    id: "ll_p13_detail_bill_request",
    title: "Detail bill request from a patient or guarantor",
    type: "playbook",
    summary: "A caller is asking for an itemized or detail bill. Confirm the right account and the approved delivery path before promising anything.",
    roles: k("billing rep", "customer service", "front desk"),
    domains: k("billing", "self-pay", "communication"),
    phases: k("stabilization week 1", "optimization weeks 2-4"),
    urgency: 2,
    escalation: 2,
    vendor_family: "epic",
    ...
    first90: [
      ...
      "Confirm the delivery method allowed by policy (portal, mail, secure email) and log a clean communication note.",
    ],
    whatToSay: [
      "'I can request the detail bill for the right visit — let me confirm the account and how you want it delivered.'",
      "'I'll log this request so anyone who picks it up after me knows what was sent.'",
    ],
    whatToCheck: [
      "Caller identity vs guarantor or proxy on the account.",
      "Whether a prior detail bill was already sent (avoid duplicates).",
      "Approved delivery method and any required note or status update.",
    ],
    whenToEscalate: "If the account spans multiple visits, has a dispute, or the detail-bill action is not visible, route customer-service / revenue-cycle owner with the account, visit, and callback.",
    keywords: k("detail bill", "itemized bill", "itemized statement", "detail bill request", "patient wants itemized bill", "caller wants detail bill", "send detail bill", "request itemized statement"),
    related_ids: ["p2", "c3", "v2"],
    sanitized_approved: true,
    status: "published",
  },
  {
    id: "ll_p13_carecompass_patient_missing",
    title: "Patient is missing from the nursing rounding / care workspace",
    type: "playbook",
    summary: "A nurse cannot see an expected patient on their rounding or care workspace — usually a relationship, unit, or assignment filter, not a chart issue.",
    roles: k("inpatient nurse", "charge nurse", "clinical support"),
    domains: k("patient list", "rounding", "assignment"),
    phases: k("cutover day 0", "stabilization week 1"),
    urgency: 3,
    escalation: 2,
    vendor_family: "epic",
    ...
    first90: [
      ...
      "Refresh once, then use the approved patient search as a fallback before calling the patient missing.",
    ],
    whatToSay: [
      "'Rounding lists are filter-driven — let me check your unit, team, and assignment view before we call the patient missing.'",
      "'If the patient is on a different unit or unassigned, that's a quick fix without a ticket.'",
    ],
    whatToCheck: [
      "Unit, team, shift, and assignment filters on the rounding view.",
      "Whether the patient has an active encounter and is on the expected unit.",
      "Whether nurse-patient relationship or assignment was actually saved.",
    ],
    whenToEscalate: "If the patient has an active encounter on this unit, is assigned, filters are clean, and they still do not appear for multiple nurses, escalate to the inpatient workflow / clinical informatics owner with unit, view, and patient context.",
    keywords: k("carecompass patient missing", "patient missing from rounding", "patient not on my list", "patient not in care workspace", "nursing rounding missing patient", "patient missing carecompass", "missing patient rounding", "patient not showing up nurse"),
    related_ids: ["p17", "c11", "v13"],
    sanitized_approved: true,
    status: "published",
  },
  {
    id: "ll_p13_iview_correction_uncharting",
    title: "Correcting or unchartering a flowsheet / interactive view entry",
    type: "playbook",
    summary: "A user filed a flowsheet or interactive-view entry in the wrong row, time column, or for the wrong patient and needs to correct or unchart it.",
    roles: k("inpatient nurse", "clinical staff"),
    domains: k("flowsheet", "documentation", "correction"),
    phases: k("cutover day 0", "stabilization week 1"),
    urgency: 3,
    escalation: 2,
    vendor_family: "epic",
    action: "modify",
    is_deep_flow: true,
    nav_trail: "Flowsheets / interactive view -> Find row + time column -> Right-click value -> Modify or unchart -> Add reason -> Verify audit",
    visual_url: "/visual-guides/iview-correction-uncharting.svg",
    visual_callouts: [
      "1 - Confirm patient, encounter, row, and time column.",
      "2 - Identify the wrong-row or wrong-time entry.",
      "3 - Right-click the value and choose modify or unchart.",
      "4 - Add the required reason — audit captures the change.",
      "5 - Verify the original value is still visible in the audit trail.",
    ],
    first90: [
      "Confirm patient, encounter, flowsheet group, row, and time column before changing anything.",
      "Right-click (or long-press) the value and choose modify or unchart per local policy; add the required reason.",
      "Verify the audit trail shows the original entry, the correction, and the user who made it.",
    ],
    whatToSay: [
      "'Corrections leave an audit trail — that's the right path, not deleting the chart.'",
      "'I'll stay with you so the reason and the corrected value are both clear.'",
    ],
    whatToCheck: [
      "Correct patient, encounter, and time column for the original entry.",
      "Whether your role can modify / unchart, or whether it requires a senior cosign.",
      "Required reason text and whether the original value remains visible in audit.",
    ],
    whenToEscalate: "If the wrong-patient entry could affect care, alerts, or downstream documents, escalate to charge nurse / clinical informatics immediately with patient, time, and what was filed.",
    keywords: k("iview correction", "iview uncharting", "uncharted flowsheet", "unchart flowsheet entry", "wrong row flowsheet", "wrong time column flowsheet", "correct flowsheet entry", "documented on wrong patient", "uncharted interactive view"),
    related_ids: ["p1", "c1", "v1"],
    sanitized_approved: true,
    status: "published",
  },
  {
    id: "ll_p13_radiant_snapboard_scheduling",
    title: "Radiology scheduling board (Snapboard) — slot, status, or patient missing",
    type: "playbook",
    summary: "The radiology scheduling board is missing a slot, patient, or status — usually a view, modality, or date filter, not a lost case.",
    roles: k("radiology scheduler", "radiology tech", "front desk"),
    domains: k("radiology", "scheduling", "snapboard"),
    phases: k("stabilization week 1", "optimization weeks 2-4"),
    urgency: 2,
    escalation: 2,
    vendor_family: "epic",
    action: "schedule",
    nav_trail: "Radiology scheduling board / snapboard -> View selector -> Modality / room / date filter -> Refresh -> Open case detail",
    visual_url: "/visual-guides/radiant-scheduling-snapboard.svg",
    visual_callouts: [
      "1 - Pick the correct board view.",
      "2 - Set modality filter (CT / MRI / etc.).",
      "3 - Refresh to pull the latest schedule.",
      "4 - Investigate any held or blocked slot.",
      "5 - Open case detail to confirm status and protocol.",
    ],
    first90: [
      "Confirm the user is on the correct radiology scheduling board view and date.",
      "Check modality, room, and resource filters — most missing slots are hidden by a filter or wrong view.",
      "Open the case detail to confirm appointment status, protocol, and any block reason before changing anything.",
    ],
    whatToSay: [
      "'The board is filter-driven — let me make sure we're looking at the right modality and date before we touch the schedule.'",
    ],
    whatToCheck: [
      "Board view, date, modality, room, and resource filters.",
      "Appointment status, protocol assignment, and patient prep state.",
      "Whether the slot is held, blocked, or already booked elsewhere.",
    ],
    whenToEscalate: "If the case is visible elsewhere but the board still shows wrong status after refresh, or a slot is locked, route the radiology scheduling owner with view, modality, and case details.",
    keywords: k("radiant scheduling", "snapboard", "radiology snapboard", "radiology board missing patient", "radiology slot missing", "radiology scheduling board", "modality board", "radiology case not on board"),
    related_ids: ["p18", "c12", "v14"],
    sanitized_approved: true,
    status: "published",
  },
  {
    id: "ll_p13_beaker_specimen_collection",
    title: "Lab specimen collection — order, label, or collection status issue",
    type: "playbook",
    summary: "A lab specimen will not print a label, will not collect, or shows the wrong status. Confirm order, specimen, and collection state before re-collecting.",
    roles: k("phlebotomist", "inpatient nurse", "lab tech"),
    domains: k("lab", "specimen", "collection"),
    phases: k("cutover day 0", "stabilization week 1"),
    urgency: 3,
    escalation: 3,
    ...
    action: "scan",
    is_deep_flow: true,
    nav_trail: "Patient chart -> Collect specimens / specimen activity -> Open the order -> Label / collect action -> Verify status",
    visual_url: "/visual-guides/beaker-specimen-collection.svg",
    visual_callouts: [
      "1 - Open the collect-specimens activity assigned to you.",
      "2 - Confirm label status — printed, not voided, not expired.",
      "3 - Scan and collect, confirming the correct printer / device.",
      "4 - Verify status moves to collected and received.",
      "5 - Never re-collect a verified specimen — escalate to lab owner first.",
    ],
    first90: [
      "Open the patient and the collect-specimens activity; confirm the order is active and assigned to this collector.",
      "Check label status — printed, voided, expired — and confirm the printer / device before reprinting.",
      "Collect, scan, and verify the status moves to collected; never re-collect a verified specimen without lab approval.",
    ],
    whatToSay: [
      "'Before we re-collect, I'm confirming the order and the existing label so we don't create a duplicate or safety risk.'",
    ],
    whatToCheck: [
      "Order status, specimen type, container, and required volume.",
      "Label print status, printer / device, and whether a label already exists at bedside.",
      "Whether the specimen has already been received or verified at the lab.",
    ],
    whenToEscalate: "If a specimen is missing, mislabeled, or already verified and a correction is needed, stop and escalate to lab / specimen processing owner immediately with order, accession, and patient timing.",
    keywords: k("beaker specimen collection", "specimen collection", "specimen label not printing", "cannot collect specimen", "specimen status wrong", "lab collection issue", "phlebotomy collection", "specimen will not collect"),
    related_ids: ["p1", "c1", "v1"],
    sanitized_approved: true,
    status: "published",
  },
  {
    id: "ll_p13_beaker_verified_result_correction",
    title: "Verified lab result needs correction or amendment",
    type: "playbook",
    summary: "A lab result has been verified and released but needs correction — this is a lab-owned amendment, never a re-collect or chart edit.",
    roles: k("lab tech", "lab supervisor", "provider"),
    domains: k("lab", "results", "correction"),
    phases: k("stabilization week 1", "optimization weeks 2-4"),
    urgency: 4,
    escalation: 4,
    vendor_family: "epic",
    action: "modify",
    is_deep_flow: true,
    nav_trail: "Result inquiry / lab result detail -> Confirm verified status -> Lab amendment / correction path -> Notify ordering provider",
    visual_url: "/visual-guides/beaker-verified-result-correction.svg",
    visual_callouts: [
      "1 - Confirm the result is verified and released (not preliminary).",
      "2 - Do not edit the chart directly — verified results are lab-owned.",
      "3 - Open the lab amendment / correction path.",
      "4 - Add the required amendment reason.",
      "5 - Notify the ordering provider so the chart and audit stay in sync.",
    ],
    first90: [
      "Confirm the result is verified and released, and capture the specific value or comment that needs correction.",
      "Use the lab-owned amendment / correction path — never edit the chart directly to change a verified result.",
      "Notify the ordering provider and any acknowledging clinician so they see the amendment in real time.",
    ],
    whatToSay: [
      "'A verified result is lab-owned — let me route this through the amendment path so the audit and providers stay in sync.'",
    ],
    whatToCheck: [
      "Verified vs preliminary status of the result.",
      "Whether the result has been acknowledged or acted on clinically.",
      "Required amendment reason and notification path to the ordering provider.",
    ],
    whenToEscalate: "If the verified result affects active clinical decisions or has already been acted on, escalate to lab supervisor and clinical owner immediately with patient timing and what is being corrected.",
    keywords: k("verified result correction", "amend lab result", "verified lab result wrong", "correct released result", "lab amendment", "result correction", "beaker verified result correction", "amend verified result"),
    related_ids: ["p1", "c1", "v1"],
    sanitized_approved: true,
    status: "published",
  },
  {
    id: "ll_p13_haiku_clinical_photo",
    title: "Mobile clinical photo will not attach or upload to the chart",
    type: "playbook",
    summary: "A clinician took a clinical photo on a mobile app and it will not attach, upload, or appear on the chart — usually context, permissions, or sync.",
    roles: k("provider", "clinical staff", "wound nurse"),
    domains: k("mobile", "media", "photo"),
    phases: k("stabilization week 1", "optimization weeks 2-4"),
    urgency: 3,
    escalation: 2,
    vendor_family: "epic",
    action: "document",
    nav_trail: "Mobile app -> Open the correct patient / encounter -> Camera / media action -> Confirm category and consent -> Sync and verify in chart",
    first90: [
      "Confirm the user is on the correct patient and encounter inside the mobile app before opening the camera.",
      "Take the photo through the in-app camera action (not the device camera roll), pick the approved media category, and confirm any required consent.",
      "Force a sync and verify the image appears under media on the chart from a desktop session.",
    ],
    whatToSay: [
      "'Clinical photos have to go through the in-app camera so they attach to the right encounter — let's confirm the patient and category first.'",
    ],
    whatToCheck: [
      "App permissions for camera, photos, and network.",
      "Whether the photo was taken inside the app or from the device camera roll.",
      "Sync status and whether the image appears in chart media from a desktop session.",
    ],
    whenToEscalate: "If photos are not syncing for multiple users, or a patient-safety image is stuck, escalate to mobile / clinical informatics with device, app version, patient timing, and what was attempted.",
    keywords: k("haiku clinical photo", "clinical photo not attaching", "mobile photo not uploading", "wound photo not attaching", "photo not in chart", "mobile clinical photo upload", "in-app camera photo", "clinical photo sync"),
    related_ids: ["p1", "c1", "v1"],
    sanitized_approved: true,
    status: "published",
  },
